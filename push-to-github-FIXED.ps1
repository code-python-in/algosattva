...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-ErrorCustom "Git is not installed. Please install Git from https://git-scm.com/"
    exit 1
}
...
if (-not (Test-Path ".git")) {
    Write-ErrorCustom "Not in a Git repository. Please run this script from the openalgo root directory."
    exit 1
}
...
if ($LASTEXITCODE -eq 0 -and $remotes) {
    ...
    Write-WarningCustom "No remote found. Adding origin..."
    $newUrl = "https://github.com/$GitHubUsername/$RepositoryName.git"
...
if ([string]::IsNullOrEmpty($status)) {
    Write-WarningCustom "No changes to commit"
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-ErrorCustom "Failed to stage changes"
    exit 1
}
...
if ($proceedWithCommit -eq 'y') {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-ErrorCustom "Failed to create commit"
        exit 1
    }
} else {
...
if ($proceedWithPush -eq 'y') {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-ErrorCustom "Failed to push to GitHub"
        Write-Host ""
