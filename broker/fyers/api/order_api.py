...
def place_gtt_order_api(data, auth):
    """
    Place a GTT (Good Till Triggered) OCO order using Fyers API v3.

    For Fyers GTT OCO orders:
    - leg1: Target order (trigger price should be ABOVE current LTP for exit)
    - leg2: Stoploss order (trigger price should be BELOW current LTP for exit)

    Args:
        data: GTT order details including:
            - symbol: Trading symbol
            - exchange: Exchange (NSE, NFO, etc.)
            - product: Product type (NRML, MIS, CNC)
            - action: Entry action (BUY or SELL)
            - quantity: Order quantity
            - sl_price: Stoploss price (trigger and limit price)
            - target_price: Target price (trigger and limit price)
            - order_type: Should be 'GTT'
        auth: Authentication token

    Returns:
        tuple: (response object, response data, gtt_order_id)
    """
    try:
        client = get_httpx_client()
        ...
        entry_action = data['action'].upper()
        ...
        # For BUY entry, exit is SELL (-1)
        # For SELL entry, exit is BUY (1)
        exit_side = -1 if entry_action == 'BUY' else 1

        # Fyers GTT OCO order structure (API v3 format)
        # For OCO orders:
        # - leg1 trigger price should be ABOVE current LTP (target for BUY entry, SL for SELL entry)
        # - leg2 trigger price should be BELOW current LTP (SL for BUY entry, target for SELL entry)
        
        if entry_action == 'BUY':
            # For BUY entry: Target is above LTP (leg1), SL is below LTP (leg2)
            gtt_payload = {
                "side": exit_side,  # -1 for SELL exit
                "symbol": symbol,
                "productType": product,
                "orderInfo": {
                    "leg1": {
            ...
            # For SELL entry: SL is above LTP (leg1), Target is below LTP (leg2)
            gtt_payload = {
                "side": exit_side,  # 1 for BUY exit
                "symbol": symbol,
                "productType": product,
                "orderInfo": {
                    "leg1": {
        ...
        logger.info(f"Placing GTT OCO order with payload: {json.dumps(gtt_payload, indent=2)}")

        # Make the POST request for GTT order (API v3 uses POST)
        response = client.post(url, headers=headers, json=gtt_payload)
        response_data = response.json()
        ...
        # Parse the response
        if response_data.get('s') == 'ok':
            gtt_order_id = response_data.get('id')
            logger.info(f"GTT OCO order placed successfully. GTT Order ID: {gtt_order_id}")
        else:
            gtt_order_id = None
            error_msg = response_data.get('message', 'Unknown error')
            logger.error(f"GTT order placement failed: {error_msg}")
            logger.debug(f"Failed GTT payload: {json.dumps(gtt_payload, indent=2)}")
            logger.debug(f"Failed GTT response: {json.dumps(response_data, indent=2)}")

        return response, response_data, gtt_order_id

    except httpx.HTTPError as e:
        logger.error(f"HTTP error during GTT order placement: {e}")
        response = type('obj', (object,), {'status_code': 500, 'status': 500})
        return response, {"s": "error", "message": f"HTTP error: {e}"}, None
    except json.JSONDecodeError as e:
        logger.error(f"JSON decode error during GTT order placement: {e}")
        response = type('obj', (object,), {'status_code': 500, 'status': 500})
        return response, {"s": "error", "message": f"Invalid JSON response: {e}"}, None
    except Exception as e:
        logger.exception(f"Error during GTT order placement: {e}")
        response = type('obj', (object,), {'status_code': 500, 'status': 500})
        return response, {"s": "error", "message": f"Error: {e}"}, None
