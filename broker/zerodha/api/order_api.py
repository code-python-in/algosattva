...
def place_gtt_order_api(data, auth):
    """
    Place a GTT (Good Till Triggered) OCO order using Zerodha Kite API.

    Uses the two-leg OCO approach where both SL and Target are in a single GTT order.
    Only one leg will execute - when one leg is triggered, the other is automatically cancelled.

    API Endpoint: POST https://api.kite.trade/gtt/triggers
    ...
    try:
        ...
        # Create two-leg OCO payload
        # Both SL and Target are in a single GTT order
        # Only one will execute - the other cancels automatically
        gtt_payload = {
            'type': 'two-leg',  # Critical: two-leg OCO
            'orders': [
                {
                    'exchange': exchange,
        ...
        logger.info(f"Placing GTT two-leg OCO order for {symbol} (SL: {sl_price}, Target: {target_price})")
        logger.debug(f"GTT payload: {gtt_payload}")

        # URL-encode the payload
        gtt_payload_encoded = urllib.parse.urlencode(gtt_payload)

        # Place the GTT two-leg OCO order
        response = client.post(
            'https://api.kite.trade/gtt/triggers',
            headers=headers,
            content=gtt_payload_encoded
        )

        response_data = response.json()
        response.status = response.status_code

        logger.debug(f"GTT response: {response_data}")

        # Check for success
        if response_data.get('status') == 'success':
            gtt_order_id = response_data.get('data', {}).get('trigger_id')
            logger.info(f"GTT two-leg OCO order placed successfully. Trigger ID: {gtt_order_id}")

            return response, response_data, gtt_order_id
        else:
            error_msg = response_data.get('data', {}).get('message', 'Failed to place GTT order')
        ...
        return response, {"status": "error", "data": {"message": f"Error: {e}"}}, None
