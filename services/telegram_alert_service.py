...
class TelegramAlertService:
    ...
    def __init__(self):
        ...
        self.alert_templates = {
            ...
            'placebracketorder': '🎯 *Bracket Order Placed*\n{details}',
    ...
    def format_order_details(self, order_type: str, order_data: Dict[str, Any], response: Dict[str, Any]) -> str:
        ...
        try:
            ...
            elif order_type == 'placebracketorder':
                details.extend([
                    f"Symbol: `{order_data.get('symbol', response.get('symbol', 'N/A'))}`",
                    f"Action: {order_data.get('action', response.get('action', 'N/A'))}",
                    f"Quantity: {order_data.get('quantity', response.get('quantity', 'N/A'))}",
                    f"Exchange: {order_data.get('exchange', 'N/A')}",
                    f"Product: {order_data.get('product', 'N/A')}",
                    f"Entry Price: {order_data.get('entry_price', response.get('entry_price', 'N/A'))}",
                    f"Stop Loss: {order_data.get('sl_price', response.get('sl_price', 'N/A'))}",
                    f"Target: {order_data.get('target_price', response.get('target_price', 'N/A'))}"
                ])
                if response.get('status') == 'success':
                    details.append(f"Entry Order ID: `{response.get('entry_order_id', 'N/A')}`")
                    if response.get('sl_order_id'):
                        details.append(f"SL Order ID: `{response.get('sl_order_id', 'N/A')}`")
                    if response.get('target_order_id'):
                        details.append(f"Target Order ID: `{response.get('target_order_id', 'N/A')}`")
                    if response.get('gtt_order_id'):
                        details.append(f"GTT Order ID: `{response.get('gtt_order_id', 'N/A')}`")
                    details.append(f"✅ {response.get('message', 'Bracket order placed')}")
                elif response.get('status') == 'partial':
                    details.append(f"Entry Order ID: `{response.get('entry_order_id', 'N/A')}`")
                    details.append(f"⚠️ {response.get('message', 'Partial execution')}")
                else:
                    details.append(f"❌ Error: {response.get('message', 'Failed')}")
