...
if [ -f "$ENV_FILE" ] && [ -r "$ENV_FILE" ] && [ -s "$ENV_FILE" ]; then
    echo "[OpenAlgo] Using existing .env file at $ENV_FILE"
else
...
EOF
        ...
        echo "[OpenAlgo] No HOST_SERVER variable found. Checking for .sample.env..."

        # Try to copy .sample.env if it exists
        if [ -f ".sample.env" ]; then
            echo "[OpenAlgo] Found .sample.env. Copying to .env..."
            cp .sample.env .env
            echo "[OpenAlgo] ✓ .env file created from .sample.env"
            echo "[OpenAlgo] Please configure your environment variables in .env"
            ENV_FILE=".env"
        else
            echo "============================================"
            echo "Error: .env file not found and .sample.env not available."
            echo ""
            echo "Solutions:"
            echo ""
            echo "1. SET HOST_SERVER ENVIRONMENT VARIABLE (Recommended for Replit):"
            echo "   In Replit Secrets, add:"
            echo "   KEY: HOST_SERVER"
            echo "   VALUE: https://your-project-name.username.replit.dev"
            echo ""
            echo "2. CREATE .env FILE MANUALLY:"
            echo "   Copy .sample.env to .env and configure your settings"
            echo ""
            echo "3. SET ALL REQUIRED ENVIRONMENT VARIABLES:"
            echo "   - HOST_SERVER (your app domain)"
            echo "   - BROKER_API_KEY"
